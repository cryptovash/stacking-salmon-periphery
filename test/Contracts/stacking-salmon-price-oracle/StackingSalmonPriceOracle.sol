...
import "./interfaces/IStackingSalmonPriceOracle.sol";

contract StackingSalmonPriceOracle is IStackingSalmonPriceOracle {
    ...
    function toUint224(uint256 input) internal pure returns (uint224) {
        require(input <= uint224(-1), "StackingSalmonPriceOracle: UINT224_OVERFLOW");
    ...
    function initialize(address uniswapV2Pair) external {
        ...
        require(
            ...
            "StackingSalmonPriceOracle: ALREADY_INITIALIZED"
        );
        ...
        returns (uint224 price, uint32 T)
    {
        ...
        require(pair.initialized, "StackingSalmonPriceOracle: NOT_INITIALIZED");
        Pair storage pairStorage = getPair[uniswapV2Pair];
        ...
        require(T >= MIN_T, "StackingSalmonPriceOracle: NOT_READY"); //reverts only if the pair has just been initialized
